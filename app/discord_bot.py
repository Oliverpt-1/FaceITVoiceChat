...
from app.config import (
    ...
    FACEIT_CLIENT_ID, FACEIT_REDIRECT_URI, LOBBY_VC_ID
)
...
@bot.tree.command(name="test-vc", description="Test: Create a VC and move yourself into it")
async def test_vc_command(interaction: discord.Interaction):
    """Test command to create a voice channel and move the user."""
    await interaction.response.defer(ephemeral=True)
    ...
    # Check if user is in a voice channel (preferably lobby)
    if not interaction.user.voice or not interaction.user.voice.channel:
        await interaction.followup.send(
            "❌ You must be in a voice channel (preferably the lobby) to test this command.",
            ephemeral=True
        )
        return
    
    try:
        # Create test VC with your user ID
        test_match_id = f"test-{interaction.user.id}"
        ...
        if vc:
            await interaction.followup.send(
                f"✅ Created test VC: **{vc.name}**\n"
                f"Use `/cleanup-vc` while in this channel to clean it up.",
                ephemeral=True
            )
        else:
            await interaction.followup.send(
                "❌ Failed to create VC. Check bot logs for details.",
                ephemeral=True
            )
    except Exception as e:
        await interaction.followup.send(
...
@bot.tree.command(name="cleanup-vc", description="Test: Clean up the test VC you're currently in (moves back to lobby and deletes)")
async def cleanup_vc_command(interaction: discord.Interaction):
    """Test command to cleanup the voice channel the user is currently in."""
    await interaction.response.defer(ephemeral=True)
    ...
    if not interaction.user.voice or not interaction.user.voice.channel:
        await interaction.followup.send(
            "❌ You must be in a voice channel to clean it up.",
            ephemeral=True
        )
        return
    ...
    try:
        ...
        # Get lobby VC
        lobby_vc = interaction.guild.get_channel(LOBBY_VC_ID) if LOBBY_VC_ID else None
        
        # Move users back to lobby (or disconnect if no lobby)
        moved_count = 0
        for member in channel.members:
            try:
                if lobby_vc:
                    ...
                    await member.move_to(None)  # Disconnect if no lobby
                    moved_count += 1
            except Exception as move_error:
                print(f"Could not move user {member.id}: {move_error}")
        ...
        await interaction.followup.send(
            f"✅ Cleaned up VC: **{channel_name}**\n"
            f"Moved {moved_count} user(s) back to lobby.",
            ephemeral=True
        )
    except Exception as e:
        await interaction.followup.send(
